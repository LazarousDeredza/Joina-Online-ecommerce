...
public class BaseActivity extends AppCompatActivity implements AddorRemoveCallbacks {
    ...
    public List<Supplier> getSupplierTotal() {
        cartList = getCartList();
        ...
        if (cartCount() > 0) {
            for (int i = 0; i < cartList.size(); i++) {
                ...
                String supplierName=cartList.get(i).getSupplier();
                Double subTotalPrice= Double.valueOf(cartList.get(i).getSubTotal());

                boolean found=false;
                for (Supplier s:suppliers                 ) {
                    if (s.getName().equals(supplierName)){
                        found=true;
                        s.setTotalPrice(Double.valueOf(s.getTotalPrice())+ Double.valueOf(cartList.get(i).getSubTotal()));
                    }

                }

                if (!found){
                    Supplier supplier=new Supplier(supplierName,subTotalPrice);
